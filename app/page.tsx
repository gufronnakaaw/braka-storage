...
export default function Home() {
  ...
  const { data: searchResults, isLoading: searchLoading } =
    useDriveSearch(searchQuery);
  const { data: breadcrumbs, isLoading: breadcrumbsLoading } =
    useDriveBreadcrumbs(null);
  ...
  const loading = searchQuery
    ? searchLoading
    : itemsLoading || breadcrumbsLoading;
  ...
  return (
    <DashboardLayout activeView="drive" mobileTitle="Drive">
      ...
      <CreateFolderModal
        ...
        onCreate={async (name) => {
          try {
            await createFolder({ name: name.toLowerCase(), parentId: null });
